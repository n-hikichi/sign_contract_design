import { Box, Button, Checkbox, Dialog, DialogActions, DialogContent, FormControl, FormControlLabel, InputLabel, MenuItem, Radio, RadioGroup, Select, SelectChangeEvent, TextField, Typography } from "@mui/material";
...
const RegisterRepresentativeSealDialog = (props: any) => {
    ...
    const onSubmit = async () => {
        ...
        try {
            ...
            const { title, file_name, company_name, ...requestData } = values; // titleとfile_nameを除く
                ...
                <Box sx={{ bgcolor: 'grey.200' }}>
                    <Box sx={{ width: '100%' }}>
                        <Typography sx={{ backgroundColor: '#0D47A1', padding: '8px', borderRadius: '4px', color: 'white', fontWeight: 'bold', textAlign: 'center', margin: '20px', fontSize: '1.5rem' }}>
                            新規に登録する代表印情報を入力してください
                        </Typography>
                    </Box>
                    <DialogContent sx={{ bgcolor: 'white', marginLeft: '20px', marginRight: '20px', border: '1px solid lightgray', overflow: 'auto', maxHeight: '70vh' }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', '& .Mui-disabled': { color: 'black' }, marginBottom: '20px' }}>
                            <TextField
                                name="user_name"
                                label="登録名"
                                variant="standard"
                                placeholder="ブロックチェーン電子契約 代表取締役印"
                    ...
                    <DialogActions sx={{ display: 'flex', justifyContent: 'center' }}>
                        <Box sx={{ textAlign: 'right' }}>
                            <Button onClick={handleSubmit(onSubmit)} color="primary" variant="contained" sx={{ marginRight: '10px', width: '10em', '&:hover': { backgroundColor: 'darkblue' } }}
                                disabled={!(isFormValid && !((tabValue === 0 && !fileUploaded) || (tabValue === 1 && !fileCreated)))}>登録する</Button>
...
export default RegisterRepresentativeSealDialog;