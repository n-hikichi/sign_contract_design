...
interface FormInput {
    ...
    customerRepresentativeSeal_temp: string,
    selectedUserSeal?: string,
    selectedRepresentativeSeal?: string,
    approval_flow: {
        customer_pic: User,
...
const RegisterWorkFlowDialog = (props: any) => {
    ...
    const onSubmit = async () => {
        ...
        try {
            ...
            const {
                customer_notifier_temp,
                customer_approver_temp,
                customer_pic,
                customer_authorizer,
                customer_approver,
                customer_notifier,
            ...
            const approvalFlowSanitized = {
                ...approvalFlowRest,
                customer_pic: filterEmpty(omitEmptyEmail(omitFile(customer_pic))),
                customer_authorizer: filterEmpty(omitEmptyEmail(omitFile(customer_authorizer))),
                customer_approver: filterEmpty(omitFileFromArray(customer_approver).filter(filterEmpty)),
                customer_notifier: filterEmpty(omitFileFromArray(customer_notifier).filter(filterEmpty)),
            };
    ...
    const getApprovalFlowDetails = async () => {
        ...
        try {
            ...
            setValue('approval_flow.customer_pic', workflow.customer_pic);
            setValue('approval_flow.customer_authorizer', workflow.customer_authorizer);
    ...
    const handleDesignateAsRepresentative = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (isChecked) {
            ...
            // setValue('representative_seal_temp', getValues('selectedUserSeal') || '');
            ...
            setselectedCustomerRepresentativeSealType(representativeSealSelectType[0].value)
        };
    };
    ...
    // 代表印
    const [selectedCustomerRepresentativeSealType, setselectedCustomerRepresentativeSealType] = useState(representativeSealSelectType[0].value);
    ...
    const handleSelectAuthorizerChange = (event: SelectChangeEvent<string>) => {
        ...
        if (selectedCustomerRepresentativeSealType === 'useUserSeal') {
    ...
    // 代表者印選択方法変更（ラジオボタン）
    const handleSelectCustomerRepresentativeSealTypeChange = (event: SelectChangeEvent<string>) => {

        const selectedValue = event.target.value;
        setselectedCustomerRepresentativeSealType(selectedValue); // ラジオボタンに選択された値をセット
    ...
    const handleSelectApproverChange = (event: SelectChangeEvent<string>) => {
        ...
        // 選択したユーザー情報を設定する
        const user = props.userInfo.find((u: any) => u.user_name === userName);
        setValue('approval_flow.customer_approver_temp', user || null);
    };
    ...
    const handleSelectNotifierChange = (event: SelectChangeEvent<string>) => {
        ...
        // 選択したユーザー情報を設定する
        const user = props.userInfo.find((u: any) => u.user_name === userName);
        setValue('approval_flow.customer_notifier_temp', user || null);
    };
                                        ...
                                        <Box sx={{ width: '30%', minWidth: '200px', marginRight: '20px' }}>
                                            <FormControl variant="standard" sx={{ width: '100%' }}>
                                                ...
                                                <Select
                                                    ...
                                                    sx={{ fontWeight: 'bold', fontSize: '20px' }}
                                                    disabled={selectedCustomerRepresentativeSealType === 'useRepresentativeSeal'}
                                                >
                                                    {props.userInfo.map((user: any) => (
                                                        <MenuItem key={user.user_name} value={user.user_name}>
                                                            {user.user_name}
                                            ...
                                            <Box sx={{ width: '50%', minWidth: '200px', marginRight: '20px' }}>
                                                <Box sx={{ display: 'flex', width: '100%', marginLeft: '5%' }}>
                                                    <FormControl component="fieldset" sx={{ width: '100%' }}>
                                                        <RadioGroup
                                                            value={selectedCustomerRepresentativeSealType}
                                                            onChange={handleSelectCustomerRepresentativeSealTypeChange}
                                                            sx={{ width: '100%' }}
                                                        >
            ...
            case 2:
                return (
                    <Box sx={{ marginBottom: '20px' }}>
                        ...
                        <Box sx={{ backgroundColor: 'white', padding: '20px', borderRadius: '4px', marginLeft: '20px', marginRight: '20px', height: '670px', overflowY: 'auto' }}>
