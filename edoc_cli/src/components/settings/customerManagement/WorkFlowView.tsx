...
interface FormInput {
    ...
    customerRepresentativeSeal_temp: string, // 画面表示用一時退避データ（代表印情報利用用）
...
const WorkFlowView = (props: any) => {
    ...
    const { control, setValue, getValues, handleSubmit } = useForm<FormInput>(
        {
            defaultValues: {
                ...
                customerRepresentativeSeal_temp: '',
                selectedUserSeal: '',
                selectedRepresentativeSeal: '',
                approval_flow: {
                    customer_pic: initialUser,
                    customer_approver: initialUsers,
                    customer_approver_temp: initialUser, // 登録リクエストを送信する際に削除する
                    customer_authorizer: initialUser,
                    customer_notifier: initialUsers,
                    customer_notifier_temp: initialUser, // 登録リクエストを送信する際に削除する
                }
            }
        }
    );
    ...
    async function fetchGetApprovalFlowData(requestInfo: any) {
        ...
        try {
            ...
            setValue('approval_flow.customer_pic', {
                user_name: workflow?.customer_pic?.user_name || '',
                position: workflow?.customer_pic?.position || '',
                email: workflow?.customer_pic?.email || '',
            });

            setValue('approval_flow.customer_authorizer', {
                user_name: workflow?.customer_authorizer?.user_name || '',
                position: workflow?.customer_authorizer?.position || '',
                email: workflow?.customer_authorizer?.email || '',
            });

            setWorkFlowApprovalList(workflow?.customer_approver || []);
            setWorkFlowNotifierList(workflow?.customer_notifier || []);
        } catch (error) {
    ...
    const handleModifyFlow = async () => {
        ...
        try {
            ...
            const { customer_approver_temp, customer_notifier_temp, ...approvalFlowRest } = approval_flow;
    ...
    const handleDesignateAsRepresentative = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (event.target.checked) {
            setValue('approval_flow.customer_authorizer', getValues('approval_flow.customer_pic'));
            // setValue('representative_seal_temp', getValues('approval_flow.customer_pic.file') || '');
            ...
            setValue('approval_flow.customer_authorizer', initialUser);
            setValue('selectedUserSeal', '');
            setValue('representative_seal_temp', '');
            setValue('customerRepresentativeSeal_temp', '');
    ...
    const handleSelectPicChange = (event: SelectChangeEvent<string>) => {
        ...
        if (user) {
            ...
            setValue('approval_flow.customer_pic', { user_name, position, email });

            // メールアドレスバリデーション
            const emailFieldName = 'approval_flow.customer_pic.email';
    ...
    const handleSelectAuthorizerChange = (event: SelectChangeEvent<string>) => {
        ...
        if (user) {
            ...
            setValue('approval_flow.customer_authorizer', { user_name, position, email });

            // メールアドレスバリデーション
            const emailFieldName = 'approval_flow.customer_authorizer.email';
            setTouched(prev => ({ ...prev, [emailFieldName]: true }));
            let error = '';
            if (email && !validator.isEmail(email)) {
                error = "メールアドレスの形式が正しくありません。";
    ...
    const handleSelectChange_selectType_customer = (event: SelectChangeEvent<string>) => {
        ...
        if (selectedValue === 'useRepresentativeSeal') {
            setValue('representative_seal_temp', getValues('customerRepresentativeSeal_temp') || '');
        };

        // if (selectedValue === 'useUserSeal') {
        //     if (isChecked_customerPic) {
        //         setValue('representative_seal_temp', selectedCustomerSeal || '');
        //     } else {
        ...
        // if (selectedValue === 'useRepresentativeSeal') {
        //     setValue('representative_seal_temp', selectedCustomerRepresentativeSealFile || '');
    ...
    const handleSelectRepresentativeSealChange = (event: SelectChangeEvent<string>) => {
        ...
        if (user) {
            ...
            // 選択した代表印を退避
            setValue('customerRepresentativeSeal_temp', user.file || null);
        };
    };
    ...
    const handleSelectApproverChange = (event: SelectChangeEvent<string>) => {
        ...
        // 選択したユーザー情報を取得してstateにセット
        const user = props.userInfo.find((u: any) => u.user_name === userName);
        // setSelectedApproverUser(user || null);

        const { user_name, position, email } = user || {};

        if (user) {
            setValue('approval_flow.customer_approver_temp', { user_name, position, email });

            // メールアドレスバリデーション
            const emailFieldName = 'approval_flow.customer_approver_temp.email';
            setTouched(prev => ({ ...prev, [emailFieldName]: true }));
            let error = '';
            if (email && !validator.isEmail(email)) {
                error = "メールアドレスの形式が正しくありません。";
    ...
    const addCustomerApprovers = () => {
        ...
        if (!getValues('approval_flow.customer_approver_temp.user_name') || !getValues('approval_flow.customer_approver_temp.email')) {
            alert('氏名とメールアドレスは入力必須です。');
            return;
        };

        const temp = getValues('approval_flow.customer_approver_temp');
        const approver = {
        ...
        const authorizerEmail = getValues('approval_flow.customer_authorizer.email');
        ...
        setValue('approval_flow.customer_approver_temp', initialUser);

        // // setWorkFlowApprovalList((prev) => [...prev, selectedApproverUser]);
        // setWorkFlowApprovalList((prev) => {
        //     if (prev.some(user => user.email === selectedApproverUser.email)) {
        //         alert(`このメールアドレス（${selectedApproverUser.email}）のユーザーは既に承認フローに追加されています。`);
        //         return prev;
        //     };
        //     return [...prev, selectedApproverUser];
        // });
    ...
    const handleSelectNotifierChange = (event: SelectChangeEvent<string>) => {
        ...
        if (user) {
            setValue('approval_flow.customer_notifier_temp', { user_name, position, email });

            // メールアドレスバリデーション
            const emailFieldName = 'approval_flow.customer_notifier_temp.email';
            setTouched(prev => ({ ...prev, [emailFieldName]: true }));
            let error = '';
            if (email && !validator.isEmail(email)) {
                error = "メールアドレスの形式が正しくありません。";
    ...
    const addCustomerNotifier = () => {
        ...
        if (!getValues('approval_flow.customer_notifier_temp.user_name') || !getValues('approval_flow.customer_notifier_temp.email')) {
            alert('氏名とメールアドレスは入力必須です。');
            return;
        };

        const temp = getValues('approval_flow.customer_notifier_temp');
        const notifier = {
            user_name: temp.user_name,
            position: temp.position,
            email: temp.email,
        ...
        setValue('approval_flow.customer_notifier_temp', initialUser);

        // setWorkFlowNotifierList(prevList => [...prevList, selectedNotifierUser]);
        // setWorkFlowNotifierList((prev) => {
        //     if (prev.some(user => user.email === selectedNotifierUser.email)) {
        //         alert(`このメールアドレス（${selectedNotifierUser.email}）のユーザーは既に通知先に追加されています。`);
        //         return prev;
        //     };
        //     return [...prev, selectedNotifierUser];
        // });
    ...
    const handleComformationDialogOpen = () => {
        ...
        setValue('approval_flow.customer_approver', workFlowApprovalList);
        setValue('approval_flow.customer_notifier', workFlowNotifierList);
    };
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '95vh', marginTop: '2.5vh' }}>
                    {isModify === 'DETAILS' && (
                        <Box sx={{ padding: '20px', backgroundColor: 'grey.200', borderRadius: '4px', boxShadow: 24, width: '100%', maxWidth: 'xl', height: '100%' }}>
                            ...
                            <Box sx={{ height: '80%', width: '100%', overflowY: 'auto' }}>
                                <Box bgcolor="white" sx={{ width: '100%', display: 'flex', flexDirection: 'column', padding: '20px', marginBottom: '10px', borderRadius: '4px', border: '1px solid lightgray' }}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'flex-start', width: '100%', padding: '10px', borderRadius: '4px' }}>
                                        <Box sx={{ width: '100%' }}>
                                            <TextField
                                                value={getValues('approval_flow.customer_pic.user_name') || '---'}
                                                label="氏名"
                                                variant="standard"
                                            ...
                                            />
                                            <TextField
                                                value={getValues('approval_flow.customer_pic.position') || '---'}
                                                label="役職"
                                                variant="standard"
                                                sx={readOnlyTextFieldStyle}
                                                disabled={true}
                                            ...
                                            />
                                            <TextField
                                                value={getValues('approval_flow.customer_pic.email') || '---'}
                                                label="メールアドレス"
                                                variant="standard"
                                                sx={readOnlyTextFieldStyle}
                                                disabled={true}
                                ...
                                <Box bgcolor="white" sx={{ width: '100%', display: 'flex', flexDirection: 'column', padding: '20px', marginBottom: '10px', borderRadius: '4px', border: '1px solid lightgray' }}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'flex-start', width: '100%', padding: '10px', borderRadius: '4px' }}>
                                        <Box sx={{ width: '70%' }}>
                                            <TextField
                                                value={getValues('approval_flow.customer_authorizer.user_name') || '---'}
                                                label="氏名"
                                                variant="standard"
                                                sx={readOnlyTextFieldStyle}
                                                disabled={true}
                                            ...
                                            />
                                            <TextField
                                                value={getValues('approval_flow.customer_authorizer.position') || '---'}
                                                label="役職"
                                                variant="standard"
                                                sx={readOnlyTextFieldStyle}
                                                disabled={true}
                                            ...
                                            />
                                            <TextField
                                                value={getValues('approval_flow.customer_authorizer.email') || '---'}
                                                label="メールアドレス"
                                                variant="standard"
                                                sx={readOnlyTextFieldStyle}
                                                disabled={true}
                                ...
                                <Box bgcolor="white" sx={{ width: '100%', display: 'flex', flexDirection: 'column', padding: '20px', marginBottom: '10px', borderRadius: '4px', border: '1px solid lightgray' }}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'flex-start', width: '100%', padding: '10px', borderRadius: '4px' }}>
                                        {(workFlowApprovalList.length === 0 &&
                                            !(
                                                (getValues('approval_flow.customer_authorizer.user_name') || '').trim() ||
                                                (getValues('approval_flow.customer_authorizer.position') || '').trim() ||
                                                (getValues('approval_flow.customer_authorizer.email') || '').trim()
                                            )
                                        ) ? (
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
                                {/* <Button onClick={handleEditDialogOpen} color="error" variant="contained" sx={{ marginRight: '10px', width: '12em', '&:hover': { backgroundColor: 'darkred' } }} >修正する</Button>
                                <Button onClick={handleDeleteDialogOpen} color="error" variant="contained" sx={{ marginRight: '10px', width: '12em', '&:hover': { backgroundColor: 'darkred' } }} >削除する</Button> */}
                    ...
                    {isModify === 'MODIFY' && (
                        <Box sx={{ padding: '20px', backgroundColor: 'grey.200', borderRadius: '4px', boxShadow: 24, width: '100%', maxWidth: 'xl', height: '100%' }}>
                            ...
                            <Box sx={{ height: '600px', width: '100%', overflowY: 'auto', border: '1px solid lightgray', backgroundColor: 'white' }}>
                                ...
                                <TabPanel value={createNewAgreementValue} index={2} dir={createNewAgreementTheme.direction}>
                                    ...
                                    <Box bgcolor="grey.200" sx={{ display: 'flex', justifyContent: 'center', width: '100%', marginTop: '20px', marginBottom: '20px', padding: '10px', borderRadius: '4px' }}>
                                        ...
                                        <Button variant="contained" color="primary" sx={{ width: '8em', marginRight: '10px', '&:hover': { backgroundColor: 'darkblue' } }} onClick={clearTempCustomerApprover} disabled={workFlowApprovalList.length === 0}>クリア</Button>
                                    </Box>
                                ...
                                <TabPanel value={createNewAgreementValue} index={3} dir={createNewAgreementTheme.direction}>
                                    ...
                                    <Box bgcolor="grey.200" sx={{ display: 'flex', justifyContent: 'center', width: '100%', marginTop: '20px', marginBottom: '20px', padding: '10px', borderRadius: '4px' }}>
                                        <Button variant="contained" color="primary" sx={{ width: '8em', marginRight: '10px', '&:hover': { backgroundColor: 'darkblue' } }} onClick={addCustomerNotifier} >追加する</Button>
                                        <Button variant="contained" color="primary" sx={{ width: '8em', marginRight: '10px', '&:hover': { backgroundColor: 'darkblue' } }} onClick={clearTempCustomerNotifier} disabled={workFlowNotifierList?.length == 0}>クリア</Button>
                                    </Box>
                                    <Box bgcolor='white' sx={{ marginBottom: '20px' }}>
                                        {workFlowNotifierList?.length == 0 ? (
                                            <Box sx={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'end' }}>
                            ...
                            {/* <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
                                <Button onClick={handleComformationDialogOpen} color="primary" variant="contained" sx={{ marginRight: '10px', width: '12em', '&:hover': { backgroundColor: 'darkblue' } }} disabled={!isFormValid} >入力内容を確認する</Button>
                    ...
                    {isModify === 'CONFIRM' && (
                        <Box sx={{ padding: '20px', backgroundColor: 'grey.200', borderRadius: '4px', boxShadow: 24, width: '100%', maxWidth: 'xl', height: '100%' }}>
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: '10px', marginBottom: '10px' }}>
                                ...
                                {/* <Button onClick={handleEditDialogClose} color="primary" variant="contained" sx={{ width: '10em', '&:hover': { backgroundColor: 'darkblue' } }}>キャンセル</Button> */}
                            </Box>
                        </Box>
                    )}
                </Box>
            ...
            < div >
                <Modal open={deleteDialogOpen}>
                    <Box sx={{ ...resendSighUrlDialogStyleConcluded_one, backgroundColor: 'grey.200', position: 'relative', height: '20vh' }}>
                        <Typography sx={{ padding: '8px', borderRadius: '4px', color: 'darkred', fontWeight: 'bold', textAlign: 'center', fontSize: '1.5em', marginBottom: '50px' }}>
                            承認フローを削除します。よろしいですか？
                        </Typography>
                        <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
                            <Button onClick={handleDeleteFlow} color="error" variant="contained" sx={{ marginRight: '10px', width: '12em', '&:hover': { backgroundColor: 'darkred' } }} >実行する</Button>
                        ...
                        <Box sx={{ backgroundColor: 'white', padding: '20px', marginBottom: '20px', border: '1px solid lightgray', overflow: 'auto', maxHeight: '70vh' }}>
                            <Box sx={{ backgroundColor: 'white', paddingTop: '20px', marginBottom: '20px' }}>
                                <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: '20px', width: '100%', height: '100%' }}>
                                    <img src={`data:image/png;base64,${getValues('representative_seal_temp')}`} alt="user" style={{ maxWidth: '30%', height: 'auto', border: '1px solid gray', borderRadius: '10px' }} />
                                </Box>
                            </Box>
                        </Box>
                        <Box sx={{
