/**
 * ModernDiscardDocumentPage - 破棄済み契約書詳細画面（Modern UI版）
 */
...
import { Download, Description, Block } from '@mui/icons-material';
...
const ModernDiscardDocumentPage: React.FC = () => {
  ...
  const tabItems = [{ label: '登録情報', icon: <Description /> }];

  return (
    <>
      <Box sx={{ display: 'flex', pt: '64px' }}>
        {/* サイドバー: DiscardSignatureHistory */}
        <DiscardSignatureHistory
        ...
        >
          <ModernDetailPageLayout loading={isLoading} showSidebar={false}>
            {/* 破棄済みバナー */}
            <Paper
              sx={{
                bgcolor: 'error.main',
...
export default ModernDiscardDocumentPage;
