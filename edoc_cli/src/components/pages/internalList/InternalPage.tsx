...
const InternalPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        async function fetchGetAgreementList() {
            try {
                const res = await apiExecutor.fetchGetAgreementList(apiStatus.agreementStatus.IN_INTERNAL_FLOW.toString())
        ...
        async function fetchGetAgreement(agreementId: string) {
            try {
                ...
                // ステータスに応じて遷移先を変更する
                if (agreementInfo.status === 'INTERNAL_APPROVING') {
                    navigate(`/documentManagement/internalDocument/checkFileDetails/${agreementId}`, { state: { agreementInfo } });
                } else if (agreementInfo.status === 'INTERNAL_REMANDING') {
                    navigate(`/documentManagement/internalDocument/remandDetails/${agreementId}`, { state: { agreementInfo } });
                } else if (agreementInfo.status === 'INTERNAL_APPROVED') {
                    navigate(`/documentManagement/internalDocument/complete/${agreementId}`, { state: { agreementInfo } });
                };
