...
import { Box, Button, Grid, Modal, Stack, TextField, Tooltip, Typography } from "@mui/material";
...
import { deleteModalStyle, remandDialogStyle, restartApploveFlowModalStyle, baseContentsStyle, pdfPreviewDialogStyle } from '../../../styles/styles';
...
const InternalCompletePage = () => {
    ...
    // ダイアログを開く関数
    const openStartCustomerFlowDialog = () => {
        setApproveFlowStartDialogOpen(true);
    };

    // 相手方承認フロー開始要求
    const onStartCustomerFlow = async () => {
        openExecuteApiDialogDialog();

        // 承認依頼先として担当者を設定する
        const approvalRequestAddress = approveFlowData.customer_pic;
    ...
    if (isLoading) {
        ...
        return (
            <>
                <Box sx={{ bgcolor: isInternalPicUser ? '#eeffee' : 'grey.200', height: 'auto', minHeight: 'calc(100vh - 35px)', paddingTop: '80px' }}>
                    <Header />
                                        ...
                                        <Box sx={{ width: '30%', justifyContent: 'flex-end', display: 'flex' }}>
                                            <AppBar position="static">
                                                <Tabs
                                                    ...
                                                    sx={{
                                                        '& .MuiTab-root': {
                                                            backgroundColor: 'lightgreen', // デフォルトの背景色
                                                        },
                                                        '& .Mui-selected': {
                                                            backgroundColor: 'darkgreen', // 選択されたタブの背景色
                                                ...
                                                >
                                                    <Tab label={<Typography sx={{ fontSize: '1.2rem', fontWeight: 'bold' }}>承認者情報</Typography>}
                                                        {...a11yProps(0)}
                                                    />
                                                    <Tab label={<Typography sx={{ fontSize: '1.2rem', fontWeight: 'bold' }}>契約基本情報</Typography>}
                                                        {...a11yProps(1)}
                                                    />
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px', width: '100%' }}>
                                <Box sx={{ width: '100%' }}>
                                    <TabPanel value={tabValue} index={0} dir={theme.direction}>
                                        {/* 今現在の承認権限を持っているユーザーに対して表示する */}
                                        <NotificationInfo
                                            present_approver={presentApprover}
                                            isPresentApprover={isPresentApprover}
                                        />
                                        <Box sx={{ position: 'relative', display: 'flex', justifyContent: 'center', alignContent: 'end', width: '100%', height: 'auto', marginBottom: '20px' }}>
                                            {isInternalPicUser && (
                                                <Button variant="contained" color="success" sx={{ '&:hover': { backgroundColor: 'darkgreen' } }} onClick={(openStartCustomerFlowDialog)}>相手方承認フローを開始する</Button>
                                            )}
                                        </Box>
                                    </TabPanel>
                    ...
                    >
                        <Box sx={{ ...deleteModalStyle, backgroundColor: '#ffeeee' }} >
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                                <Button variant="contained" color="error" sx={{ '&:hover': { backgroundColor: 'darkred' }, width: '10em', marginRight: '5px' }} onClick={(onDelete)}>実行する</Button>
                                <Button variant="contained" color="primary" sx={{ '&:hover': { backgroundColor: 'darkblue' }, width: '10em' }} onClick={(handleDeleteDialogClose)}>キャンセル</Button>
                ...
                <div>
                    <Modal
                        ...
                        aria-describedby="modal-modal-description"
                    >
                        <Box sx={{ position: 'absolute' as 'absolute', top: '50%', left: '50%', width: '50%', height: '50%', transform: 'translate(-50%, -50%)', bgcolor: '#eeffee', border: '2px solid #000', boxShadow: 24, p: '15px', overflowY: 'auto' }}>
                            <Typography sx={{ backgroundColor: 'darkgreen', padding: '8px', borderRadius: '4px', color: 'white', fontWeight: 'bold', textAlign: 'center', fontSize: '1.5em', marginBottom: '20px' }}>
                                相手方承認フローを開始します。よろしいですか？
                            </Typography>
                            <Box>
                                <Box sx={{ marginBottom: '20px' }}>
                                    <Typography sx={{ backgroundColor: 'lightblue', padding: '8px', borderRadius: '4px', fontWeight: 'bold', width: '40%', border: '1px solid lightgray', fontSize: '20px' }}>
                                        承認依頼送信先
                                    </Typography>
                                    <Box bgcolor='white' sx={{ border: '1px solid lightgray', padding: '20px', paddingTop: '30px', paddingBottom: '30px' }}>
                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', '& .Mui-disabled': { color: 'black' }, marginBottom: '20px' }}>
                                            <TextField
                                                value={customerFirstApprover?.company_name}
                                                label="会社名"
                                                variant="standard"
                                        ...
                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', '& .Mui-disabled': { color: 'black' }, marginBottom: '20px' }}>
                                            <TextField
                                                value={customerFirstApprover?.user_name}
                                                label="ユーザー名"
                                                variant="standard"
                                                sx={readOnlyTextFieldStyle}
                                                disabled={true}
                                        ...
                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', '& .Mui-disabled': { color: 'black' } }}>
                                            <TextField
                                                value={customerFirstApprover?.email}
                                                label="メールアドレス"
                                                variant="standard"
                                                sx={readOnlyTextFieldStyle}
                                                disabled={true}
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                                <Button variant="contained" color="success" sx={{ width: '10em', marginRight: '10px', '&:hover': { backgroundColor: 'darkgreen' } }} onClick={onStartCustomerFlow} disabled={false}>開始する</Button>
                                <Button variant="contained" color="primary" sx={{ width: '10em', marginRight: '10px', '&:hover': { backgroundColor: 'darkblue' } }} onClick={handleApproveFlowStartDialogClose} disabled={false}>キャンセル</Button>
                            </Box>
                        </Box>
                    </Modal>
                </div>
...
export default InternalCompletePage;