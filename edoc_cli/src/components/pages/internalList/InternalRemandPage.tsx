...
/**
 * 
 * 自社ユーザー向け差戻し画面（自社ユーザー差戻し）
 * 
 */
const InternalRemandPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const [agreement, approvals, file, remandRequest] = await Promise.all(responses.map((res: Response) => res.json()));
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', paddingLeft: '5%', paddingRight: '5%', width: '100%' }} px={4}>
                            ...
                            {isInternalPicUser && (
                                <Box sx={{ width: '30%', display: 'flex', marginBottom: '5px' }}>
                                    <Button variant="contained" color="error" sx={{ width: '12em', marginRight: '10px', '&:hover': { backgroundColor: 'darkred' } }} onClick={openDeleteDialog} >破棄する</Button>
                                </Box>
                            )}
...
export default InternalRemandPage;