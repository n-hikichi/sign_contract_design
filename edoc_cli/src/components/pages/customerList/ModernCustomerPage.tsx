/**
 * ModernCustomerPage - MUI Default スタイル相手方承認中文書一覧画面
 */
    ...
    async function fetchGetAgreementList() {
      try {
        const res = await apiExecutor.fetchGetAgreementList(
          apiStatus.agreementStatus.IN_CUSTOMER_FLOW.toString()
        );
        if (res.status !== api.HTTP_OK) {
          setErrorCode(res.status);
          setErrorProcess('相手方承認フロー文書取得処理');
  ...
  return (
    <>
      <ModernPageLayout
        loading={loading}
        title="相手方承認中"
        subtitle="相手方の承認待ち書類一覧です"
        breadcrumbs={[
          { label: '新規契約書管理', path: '/documentManagement/register' },
          { label: '相手方承認中' },
        ]}
        stepper={<CommonStepper activeStep={3} />}
      >
        {/* ガイダンスバナー */}
        <Paper sx={{ mb: 3, p: 2.5 }}>
