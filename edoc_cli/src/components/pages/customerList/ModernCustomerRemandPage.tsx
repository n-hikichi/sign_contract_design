/**
 * ModernCustomerRemandPage - モダナイズされた相手方差戻し中画面
 *
...
/**
 * ModernCustomerRemandPage コンポーネント
 */
const ModernCustomerRemandPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [agreement, approvals, file, remandResponse] = await Promise.all(
        ...
        // 差戻し情報設定
        if (remandResponse && remandResponse.length > 0) {
...
export default ModernCustomerRemandPage;
