...
import SignatureHistoryPdf from './SignatureHistoryPdfForGuest';
...
const OpenedSideMenu: any = (props: any) => {
    ...
    return (
        ...
        >
            ...
            <Box sx={{ flexGrow: 1, overflow: 'auto' }}>
                <>
                    <Box display="flex" sx={{ width: '100%', padding: '10px', paddingRight: '40px', paddingLeft: '15px', bgcolor: 'lightblue' }}>
                        ...
                        <Box display="flex" justifyContent="flex-end" sx={{ paddingRight: '10px' }}>
                            {isFileValid && isAgreementValid ? <FileDownloadDoneIcon sx={{ color: 'white', bgcolor: '#1565c0' }} /> : <FileDownloadDoneIcon sx={{ color: 'red' }} />}
                        </Box>
                    </Box>
                    <Accordion>
                        <AccordionSummary
                        ...
                        >
                            <Box display="flex" sx={{ width: '100%' }}>
                                ...
                                <Box display="flex" justifyContent="flex-end">
                                    {registerUser?.valid ? <FileDownloadDoneIcon sx={{ color: 'white', bgcolor: '#1565c0' }} /> : <FileDownloadDoneIcon sx={{ color: 'red' }} />}
                                </Box>
                            </Box>
                        </AccordionSummary>
                                ...
                                <Accordion>
                                    ...
                                    >
                                        <Box display="flex" sx={{ width: '100%' }}>
                                            ...
                                            <Box display="flex" justifyContent="flex-end">
                                                {signature.valid ? <FileDownloadDoneIcon sx={{ color: 'white', bgcolor: '#1565c0' }} /> : <FileDownloadDoneIcon sx={{ color: 'red' }} />}
                                            </Box>
                                        </Box>
                                    </AccordionSummary>
                                    <AccordionDetails sx={{ width: '100%' }}>
            ...
            {/* <Box sx={{ marginBottom: '70px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <DrawerFooter>
                    <PDFDownloadLink document={<SignatureHistoryPdf approveFlow_Data={props.approveFlow_Data} approveResult={approveUserList} agreement_id={props.agreement_id} title={props.title} />} fileName={`signature_report_${props.title}_${converter.getCurrentDate()}`}>
                        <Button variant='contained' size='large' onClick={() => handlePdfPreviewDialogClose()} sx={{ margin: '10px', '&:hover': { backgroundColor: 'darkblue' } }}>署名履歴ダウンロード</Button>
                    </PDFDownloadLink>
                </DrawerFooter>
            </Box> */}
        </Drawer>
...
const SignatureHistoryForGuest: any = (props: any) => {
    const [isOpen, setIsOpen] = useState(false);
    return (
        isOpen ?
            <OpenedSideMenu setIsOpen={setIsOpen} approveFlow_Data={props.approveFlow_Data} approveResult={props.approveResult} agreement_id={props.agreement_id} title={props.title} own_company={props.own_company} customer_company={props.customer_company} /> :
            <ClosedSideMenu setIsOpen={setIsOpen} />
    );
}

export default SignatureHistoryForGuest;