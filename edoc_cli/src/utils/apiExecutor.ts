...
export namespace apiExecutor {
    ...
    // 
    // ユーザー情報を取得
    export const fetchGetRepresentativeSealImage = async (companyId: string, workflowId: string): Promise<Response> => {
        return await api.getRepresentativeSealImage(companyId, workflowId);
    };
    export const fetchGetConcludedRepresentativeSealImage = async (agreementId: string): Promise<Response> => {
        return await api.getRepresentativeSeal(agreementId);
    };
    export const fetchGetConcludedRepresentativeSeal = async (agreementId: string): Promise<Response> => {
        return await api.getConcludedRepresentativeSealImage(agreementId);
    };
    // export const fetchGetRepresentativeSeal = async (type: string): Promise<Response> => {
    //     return await api.getRepresentativeSeal(type);
    // };
    export const fetchGetApprovalFlowList = async (companyId: string): Promise<Response> => {
        return await api.getApprovalFlowList(companyId);
    };
    export const fetchGetApprovalFlow = async (companyId: string, workflowId: string, workflowType: string): Promise<Response> => {
        return await api.getApprovalFlow(companyId, workflowId, workflowType);
    };
    // export const fetchPostApprovalFlow = async (companyId: string): Promise<Response> => {
    //     return await api.postApprovalFlow(companyId);
    // };
